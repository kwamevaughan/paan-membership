...
const LanguageSwitch = ({ mode }) => {
  ...
  useEffect(() => {
    if (typeof window === "undefined" || !scriptLoaded || !googleTranslateElRef.current) return;
  ...
  useEffect(() => {
    if (typeof window === "undefined" || !widgetInitialized || !googleTranslateElRef.current) return;
