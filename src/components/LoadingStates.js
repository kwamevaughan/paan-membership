...
export function Spinner({
  ...
  return (
    <div className={`flex flex-col items-center justify-center ${className}`}>
      <div
        className={`${sizeClasses[size]} border-4 border-gray-200 ${colorClasses[color]} rounded-full animate-spin mb-3`}
      />
      {text && (
...
export function Shimmer({ className = "", width = "w-full", height = "h-4" }) {
  return (
    <div
      className={`${width} ${height} bg-gray-200 rounded relative overflow-hidden ${className}`}
    >
...
export function LoadingOverlay({
  ...
  return (
    <div
      className={`fixed inset-0 min-h-screen z-[9999] flex items-center justify-center ${
