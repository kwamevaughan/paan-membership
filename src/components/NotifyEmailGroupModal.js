...
const NotifyEmailGroupModal = ({ isOpen, onClose, jobTitle, jobId, expiresOn, mode }) => {
    ...
    const handlePreview = async () => {
        if (selectedGroups.length === 0) {
            toast.error("Please select at least one email group to preview!", { icon: "⚠️" });
            return;
        }
        ...
        try {
            ...
            if (errors.length > 0) throw new Error(errors[0].error || "Failed to fetch some group details");
            ...
            toast.success(`Previewing ${selectedGroups.length} group(s) (${totalEmails} emails)`, {
                icon: "👀",
            });
        } catch (error) {
    ...
    const handleNotify = async () => {
        if (selectedGroups.length === 0) {
            toast.error("Please select at least one email group to notify!", { icon: "⚠️" });
            return;
        }
        ...
        try {
            const response = await fetch("/api/notify-email-group", {
                ...
                body: JSON.stringify({ groupIds: selectedGroups, jobTitle, jobId, expiresOn }),
            });
            const result = await response.json();
            if (!response.ok) throw new Error(result.error || "Failed to initiate notification");
            ...
            const checkStatus = async () => {
                const interval = setInterval(async () => {
                    const statusRes = await fetch(`/api/email-status?jobId=${result.jobId}`);
                    ...
                    if (status.status === "completed") {
                        clearInterval(interval);
                        toast.success(`All ${status.total_emails} emails sent successfully!`, {
                            id: notifyingToast,
                            duration: 5000,
                        });
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            ...
            >
                ...
                >
                    Job "{jobTitle}" has been posted. Select groups to notify:
                </p>
                <div className="mb-4">
                ...
                {isPreviewOpen && previewGroups.length > 0 && (
                    ...
                    >
                        <h3 className="text-sm font-semibold mb-2">
                            ...
                            {previewGroups
                                .reduce((acc, g) => acc.concat((g.emails || "").split(",")), [])
